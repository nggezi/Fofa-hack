...
def main():
    ...
    parser.add_argument('--fofa-key', type=str, help="fofa api key值(配合fofa终身会员使用)")
    parser.add_argument('--debug', help="fofa-hack调试模式,运行过程中输出更多运行日志", action='store_true')
    parser.add_argument('--time-type', help="fofa-hack时间类型 (day | hour)，默认为day，如果选择hour的话数据采集粒度会变成按小时的", choices=['day','hour'], default='day')
    ...
    if args.authorization_file:
        ...
        with open(config.AUTHORIZATION_FILE, 'r') as f:
            for line in f.readlines():
                config.AUTHORIZATION_LIST.append(line.strip())
    ...
    if args.keyword:
        ...
        if not icon_url.endswith(".ico"):
            icon_url = icon_url.rstrip("/") + "/favicon32.ico"
        try:
            _icon = mmh3.hash(codecs.lookup('base64').encode(requests.get(icon_url).content)[0])
    ...
    fofa = FofaMain(search_key, inputfile, filename, time_sleep, endcount, level, level_data,
                    output, output_data, fuzz, timeout, config.IS_PROXY)
    fofa.start()
